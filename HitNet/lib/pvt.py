...
# Import PVT backbone from the same package
from .pvtv2 import hitnet_pvt_v2_b2
...
class Hitnet(nn.Module):
    def __init__(self, channel=32,n_feat=32,scale_unetfeats=32,kernel_size=3,reduction=4,bias=False,act=nn.PReLU()):
        ...
        self.backbone = hitnet_pvt_v2_b2()  # [64, 128, 320, 512]
        # Resolve pretrained checkpoint relative to this file to avoid cwd issues
        root_dir = os.path.abspath(os.path.join(os.path.dirname(__file__), os.pardir))
        path = os.path.join(root_dir, 'pretrained_pvt', 'pvt_v2_b2.pth')
